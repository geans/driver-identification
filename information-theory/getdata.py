...
class GetData:
    def __init__(self, path_dataset, label_feature_name, driver_feature_name, features=config.ALL_FEATURES, trips=None):
        ...
        if trips is None:
            for i in range(1, 1 + 8):
                list_df_a.append(pd.read_csv(path_dataset + f'/A/All_{i}.csv'))
                # list_df_a_inf.append(pd.read_csv(path_dataset + f'/A/All_{i}_inf.csv'))
            for i in range(1, 1 + 8):
                list_df_b.append(pd.read_csv(path_dataset + f'/B/All_{i}.csv'))
                # list_df_b_inf.append(pd.read_csv(path_dataset + f'/B/All_{i}_inf.csv'))
            for i in range(1, 1 + 5):
                list_df_c.append(pd.read_csv(path_dataset + f'/C/All_{i}.csv'))
                # list_df_c_inf.append(pd.read_csv(path_dataset + f'/C/All_{i}_inf.csv'))
            for i in range(1, 1 + 9):
                list_df_d.append(pd.read_csv(path_dataset + f'/D/All_{i}.csv'))
        ...
        # self.__DFA_inf[self.__driver] = ['A'] * self.__DFA_inf.shape[0]
        # self.__DFB_inf[self.__driver] = ['B'] * self.__DFB_inf.shape[0]
        # self.__DFC_inf[self.__driver] = ['C'] * self.__DFC_inf.shape[0]
        # self.__DFD_inf[self.__driver] = ['D'] * self.__DFD_inf.shape[0]
        # self.__new_df = pd.DataFrame(columns=self.__DFA_inf.columns)
        # for i in range(self.__DFA_inf.shape[0]):
        #     for data in [self.__DFA_inf, self.__DFB_inf, self.__DFC_inf, self.__DFD_inf]:
        #         self.__new_df = pd.concat([self.__new_df, data[i:i+1]])
        # self.__new_df[self.__label] = [0] * self.__new_df.shape[0]
